  ...
  factory: (require) => {
    ...
    function SshSurface({ session, active, onState, onError, onChallenge }) {
      ...
      useEffect(() => {
        ...
        const terminal = new Terminal({ cursorBlink: true, fontSize: 13, lineHeight: 1.2, scrollback: 10000, theme: { background: "#101419", foreground: "#e8edf2", cursor: "#d8e6f3" } });
        ...
        void connect();
        return () => { disposed = true; terminalRef.current = null; resize.disconnect(); input.dispose(); offData(); offExit(); terminal.dispose(); if (connectionId) void window.laobosDesktop.ssh.disconnect(connectionId); };
