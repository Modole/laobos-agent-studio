...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BrowserPanel() {
      ...
      useEffect(() => {
        const show = (event) => {
          if (event.detail?.tool === "close") { setOpen(false); return; }
      ...
      useEffect(() => {
        ...
        void window.laobosDesktop.browser.show(true).then((value) => { applyBrowserState(value); sync(); }).catch((reason) => setError(messageOf(reason)));
        ...
        const focusAddress = (event) => {
          ...
          event.preventDefault(); addressInput.current?.focus(); addressInput.current?.select();
        };
        window.addEventListener("keydown", focusAddress);
        return () => {
          disposed = true; observer.disconnect(); window.removeEventListener("resize", sync); window.removeEventListener("keydown", focusAddress);
          offState(); offOps(); void window.laobosDesktop.browser.show(false);
        };
      }, [open]);

      const navigate = async () => {
        const target = addressRef.current.trim();
        if (!target) { addressInput.current?.focus(); return; }
        try { setError(""); applyBrowserState(await window.laobosDesktop.browser.navigate(target)); }
        catch (reason) { setError(messageOf(reason)); }
      };
      const action = async (name) => {
        try { applyBrowserState(await window.laobosDesktop.browser.action(name)); }
        catch (reason) { setError(messageOf(reason)); }
      };
      const toggleOps = async () => {
        if (["starting", "stopping", "external"].includes(ops.state)) return;
        try { setError(""); setOps(await (ops.state === "running" ? window.laobosDesktop.browserOps.stop() : window.laobosDesktop.browserOps.start())); }
        catch (reason) { setError(messageOf(reason)); }
      };
      ...
      return <section className="lbs-browser-panel" role="dialog" aria-label="内置浏览器">
        <header className="lbs-browser-toolbar">
          <button className="lbs-browser-nav-button" disabled={!state.canGoBack} onClick={() => action("back")} title="后退">←</button>
          <button className="lbs-browser-nav-button" disabled={!state.canGoForward} onClick={() => action("forward")} title="前进">→</button>
          <button className="lbs-browser-nav-button" onClick={() => action(state.loading ? "stop" : "reload")} title={state.loading ? "停止加载" : "刷新"}>{state.loading ? "×" : "↻"}</button>
          <label className="lbs-browser-address-shell"><span className="lbs-browser-address-icon">HTTP</span><input ref={addressInput} className="lbs-browser-address" value={address} onChange={(event) => setAddress(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void navigate(); }} placeholder="输入地址，例如 localhost:5173" aria-label="浏览器地址" /></label>
          <button className="lbs-browser-open" disabled={!address.trim()} onClick={navigate}>打开</button>
          <div className="lbs-browser-ops"><button disabled={["starting", "stopping", "external"].includes(ops.state)} onClick={toggleOps}>{opsLabel}</button><span className="lbs-browser-ops-state" data-state={ops.state} title={ops.message}>{ops.message || ops.state}</span></div>
          <button className="lbs-browser-close" onClick={close}>关闭</button>
        </header>
        {error ? <div className="lbs-browser-error"><span>{error}</span><button type="button" disabled={!address.trim()} onClick={navigate}>重试</button></div> : null}
        <div className="lbs-browser-viewport" ref={viewport} />
      </section>;
    }
